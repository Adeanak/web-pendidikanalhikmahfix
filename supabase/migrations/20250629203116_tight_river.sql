/*
  # Program Details Table and Data

  1. New Tables
     - `program_details` - Stores detailed information about educational programs
       - Includes program features, curriculum, facilities as text arrays
       - Stores fee structure (monthly, registration, uniform, book fees)
       - Stores image gallery URLs

  2. Security
     - Enables RLS on program_details table
     - Adds policies for public and authenticated access
     - Adds management policies for admins

  3. Data
     - Inserts default data for three programs (PAUD, TKA/TPA, Diniyah)
*/
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM pg_policy WHERE polname = 'Anyone can read program details' AND polrelid = 'program_details'::regclass) THEN
    DROP POLICY "Anyone can read program details" ON program_details;
  END IF;
  
  IF EXISTS (SELECT 1 FROM pg_policy WHERE polname = 'Authenticated users can read program details' AND polrelid = 'program_details'::regclass) THEN
    DROP POLICY "Authenticated users can read program details" ON program_details;
  END IF;
  
  IF EXISTS (SELECT 1 FROM pg_policy WHERE polname = 'Super admin can manage program details' AND polrelid = 'program_details'::regclass) THEN
    DROP POLICY "Super admin can manage program details" ON program_details;
  END IF;
  
  IF EXISTS (SELECT 1 FROM pg_policy WHERE polname = 'Authorized users can manage program details' AND polrelid = 'program_details'::regclass) THEN
