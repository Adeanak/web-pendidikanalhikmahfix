/*
  ...
  1. New Tables
    ...
    - `ppdb_settings`
      - `id` (integer, primary key)
      - `is_open` (boolean, registration status)
      - `start_date` (date, registration start)
      - `end_date` (date, registration end)
      - `academic_year` (text, academic year)
      - `updated_at` (timestamp)

  2. Security
    - Enable RLS on all new tables
    - Add policies for authenticated users to read/manage PPDB data
    - Add policies for super admin to manage all PPDB settings
...
-- Create sequence for ppdb_test_schedules if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'ppdb_test_schedules_id_seq') THEN
    CREATE SEQUENCE ppdb_test_schedules_id_seq;
    ALTER TABLE ppdb_test_schedules ALTER COLUMN id SET DEFAULT nextval('ppdb_test_schedules_id_seq'::regclass);
  END IF;
END $$;
